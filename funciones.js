...
const creari = (usuario) => {
    listaru();
    listar();
    let cur = listaCursos.find(existe => existe.nombre == usuario.curso)
    if (!cur) {
...
const actualizar = (id) => {
    ...
    if (!encontrado) {
        ...
        if (encontrado.estado == "disponible") {
            ...
            console.log('Se ha actualizado el estado del curso '+ id +' a cerrado');
            guardar();
        }
        else {
            encontrado.estado = "disponible";
            console.log('Se ha actualizado el estado del curso ' + id + ' a disponible');
            guardar();
        }
    }
}
